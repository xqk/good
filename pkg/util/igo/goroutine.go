	...
	go func() {
		defer func() {
			if err := recover(); err != nil {
				_logger.Error("recover", ilog.Any("err", err), ilog.String("line", fmt.Sprintf("%s:%d", file, line)))
			}
		}()
		// 忽略返回值, goroutine执行的返回值通常都会忽略掉
		_, err := inj.Invoke(fn)
		if err != nil {
			_logger.Error("inject", ilog.Any("err", err), ilog.String("line", fmt.Sprintf("%s:%d", file, line)))
			return
		}
	}()
}
