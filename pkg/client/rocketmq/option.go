...
func StdPushConsumerConfig(name string) *ConsumerConfig {
	...
	cc := RawConsumerConfig("good.rocketmq." + name + ".consumer")
	rc := RawConfig("good.rocketmq." + name)
...
func StdProducerConfig(name string) *ProducerConfig {
	pc := RawProducerConfig("good.rocketmq." + name + ".producer")
	rc := RawConfig("good.rocketmq." + name)
	// 兼容rocket_client_mq变更，addr需要携带shceme
	if len(pc.Addr) == 0 {
...
func RawConfig(key string) Config {
	...
	if err := conf.UnmarshalKey(key, &config, conf.TagName("toml")); err != nil {
		ilog.Panic("unmarshal config", ilog.String("field", key), ilog.Any("ext", config))
	}

	if idebug.IsDevelopmentMode() {
		idebug.PrettyJsonPrint(key, config)
	}
	return config
}
...
func RawConsumerConfig(key string) ConsumerConfig {
	...
	if err := conf.UnmarshalKey(key, &config); err != nil {
		ilog.Panic("unmarshal config", ilog.String("key", key), ilog.Any("config", config))
	}
...
func RawProducerConfig(key string) ProducerConfig {
	...
	if err := conf.UnmarshalKey(key, &config); err != nil {
		ilog.Panic("unmarshal config", ilog.String("key", key))
	}
