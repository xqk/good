		...
		for {
			for n := range rch {
				...
				if err := n.Err(); err != nil {
					ilog.Error(ecode.MsgWatchRequestErr, ilog.FieldErrKind(ecode.ErrKindRegisterErr), ilog.FieldErr(err), ilog.FieldAddr(prefix))
					continue
				}
				for _, ev := range n.Events {
					...
					case w.eventChan <- ev:
					default:
						ilog.Error("watch etcd with prefix", ilog.Any("err", "block event chan, drop event message"))
					}
				}
			}
			ctx, cancel := context.WithCancel(context.Background())
