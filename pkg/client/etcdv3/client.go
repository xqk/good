...
func newClient(config *Config) (*Client, error) {
	...
	config.logger = config.logger.With(ilog.FieldAddrAny(config.Endpoints))
	...
	if config.CaCert != "" {
		...
		if err != nil {
			config.logger.Panic("parse CaCert failed", ilog.Any("err", err))
		}
	...
	if config.CertFile != "" && config.KeyFile != "" {
		...
		if err != nil {
			config.logger.Panic("load CertFile or KeyFile failed", ilog.Any("config", config), ilog.Any("err", err))
		}
	...
	if err != nil {
		// config.logger.Panic("client etcd start panic", ilog.FieldMod(ecode.ModClientETCD), ilog.FieldErrKind(ecode.ErrKindAny), ilog.FieldErr(err), ilog.FieldValueAny(config))
