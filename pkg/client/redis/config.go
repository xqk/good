...
func DefaultRedisConfig() Config {
	return Config{
		...
		WriteTimeout:  itime.Duration("1s"),
		IdleTimeout:   itime.Duration("60s"),
		ReadOnly:      false,
		Debug:         false,
		EnableTrace:   false,
		SlowThreshold: itime.Duration("250ms"),
		OnDialError:   "panic",
...
func StdRedisConfig(name string) Config {
	return RawRedisConfig("good.redis." + name)
}
...
func RawRedisConfig(key string) Config {
	...
	if err := conf.UnmarshalKey(key, &config); err != nil {
		ilog.Panic("unmarshal redisConfig",
			ilog.String("key", key),
			ilog.Any("redisConfig", config),
			ilog.String("error", err.Error()))
	}
	return config
}
...
func (config Config) Build() *Redis {
	...
	if count < 1 {
		config.logger.Panic("no address in redis config", ilog.Any("config", config))
	}
...
func (config Config) buildStub() *redis.Client {
	...
	if err := stubClient.Ping().Err(); err != nil {
		...
		case "panic":
			config.logger.Panic("dial redis fail", ilog.Any("err", err), ilog.Any("config", config))
		default:
			config.logger.Error("dial redis fail", ilog.Any("err", err), ilog.Any("config", config))
		}
	}
...
func (config Config) buildCluster() *redis.ClusterClient {
	...
	if err := clusterClient.Ping().Err(); err != nil {
		switch config.OnDialError {
		case "panic":
			config.logger.Panic("start cluster redis", ilog.Any("err", err))
		default:
			config.logger.Error("start cluster redis", ilog.Any("err", err))
		}
	}
...
func StdRedisStubConfig(name string) Config {
	return RawRedisStubConfig("good.redis." + name + ".stub")
}
...
func StdRedisClusterConfig(name string) Config {
	return RawRedisClusterConfig("good.redis." + name + ".cluster")
}
