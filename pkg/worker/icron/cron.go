...
// Name ...
func (f FuncJob) Name() string { return istring.FunctionName(f) }
...
func newCron(config *Config) *Cron {
	...
	config.logger = config.logger.With(ilog.FieldMod("worker.cron"))
	cron := &Cron{
		Config: config,
		Cron: cron.New(
...
func (c *Cron) Schedule(schedule Schedule, job NamedJob) EntryID {
	...
	c.logger.Info("add job", ilog.String("name", job.Name()))
...
func (c *Cron) Run() error {
	...
	c.logger.Info("run worker", ilog.Int("number of scheduled jobs", len(c.Cron.Entries())))
	c.Cron.Run()
	return nil
}
