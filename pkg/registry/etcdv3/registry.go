...
func newETCDRegistry(config *Config) (*etcdv3Registry, error) {
	if config.logger == nil {
		config.logger = ilog.GoodLogger
	}
	config.logger = config.logger.With(ilog.FieldMod(ecode.ModRegistryETCD), ilog.FieldAddrAny(config.Config.Endpoints))
...
func (reg *etcdv3Registry) ListServices(ctx context.Context, name string, scheme string) (services []*server.ServiceInfo, err error) {
	...
	if getErr != nil {
		reg.logger.Error(ecode.MsgWatchRequestErr, ilog.FieldErrKind(ecode.ErrKindRequestErr), ilog.FieldErr(getErr), ilog.FieldAddr(target))
		return nil, getErr
	}
	...
	for _, kv := range getResp.Kvs {
		...
		if err := json.Unmarshal(kv.Value, &service); err != nil {
			reg.logger.Warnf("invalid service", ilog.FieldErr(err))
			continue
		}
...
func (reg *etcdv3Registry) Close() error {
	...
	reg.kvs.Range(func(k, v interface{}) bool {
		...
		go func(k interface{}) {
			...
			if err != nil {
				reg.logger.Error("unregister service", ilog.FieldErrKind(ecode.ErrKindRequestErr), ilog.FieldErr(err), ilog.FieldErr(err), ilog.FieldKeyAny(k), ilog.FieldValueAny(v))
			} else {
				reg.logger.Info("unregister service", ilog.FieldKeyAny(k), ilog.FieldValueAny(v))
			}
			cancel()
		}(k)
		return true
...
func (reg *etcdv3Registry) registerMetric(ctx context.Context, info *server.ServiceInfo) error {
	...
	if err != nil {
		reg.logger.Error("register service", ilog.FieldErrKind(ecode.ErrKindRegisterErr), ilog.FieldErr(err), ilog.FieldKeyAny(key), ilog.FieldValueAny(info))
		return err
	}

	reg.logger.Info("register service", ilog.FieldKeyAny(key), ilog.FieldValueAny(val))
...
func deleteAddrList(al *registry.Endpoints, prefix, scheme string, kvs ...*mvccpb.KeyValue) {
	for _, kv := range kvs {
		...
		if strings.HasPrefix(addr, "providers/"+scheme) {
			...
			if err != nil {
				ilog.Error("parse uri", ilog.FieldErrKind(ecode.ErrKindUriErr), ilog.FieldErr(err), ilog.FieldKey(string(kv.Key)))
				continue
			}
...
/*
key: /good/main/configurator/grpc:///routes/1
val:
{
	"upstream": { // 客户端配置
		"nodes": { // 按照node负载均衡
...
key: /good/main/configurator/grpc://127.0.0.1/routes/2
val:
{
	"upstream": { // 客户端配置
		"nodes": { // 按照node负载均衡
...
key: /good/main/configurator/grpc:///consumers/client-demo
val:
{
