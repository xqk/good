...
func RawConfig(key string) *Config {
	...
	if err := conf.UnmarshalKey(key, &config); err != nil &&
		...
		config.logger.Panic("http server parse config panic", ilog.FieldErrKind(ecode.ErrKindUnmarshalConfigErr), ilog.FieldErr(err), ilog.FieldKey(key), ilog.FieldValueAny(config))
	}
	return config
}
...
func (config *Config) MustBuild() *Server {
	...
	if err != nil {
		ilog.Panicf("build echo server failed: %v", err)
	}
	return server
}
