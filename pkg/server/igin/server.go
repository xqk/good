...
func newServer(config *Config) *Server {
	...
	if err != nil {
		config.logger.Panic("new igin server err", ilog.FieldErrKind(ecode.ErrKindListenErr), ilog.FieldErr(err))
	}
...
func (s *Server) Serve() error {
	// s.Gin.StdLogger = ilog.GoodLogger.StdLog()
	...
	if err == http.ErrServerClosed {
		s.config.logger.Info("close gin", ilog.FieldAddr(s.config.Address()))
		return nil
	}
