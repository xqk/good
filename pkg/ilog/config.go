...
func init() {
	conf.OnLoaded(func(c *conf.Configuration) {
		...
		if c.Get(constant.ConfigPrefix+".logger.good") != nil {
			log.Printf("reload default logger with configKey: %s", ConfigEntry("good"))
			GoodLogger = RawConfig(constant.ConfigPrefix + ".logger.good").Build()
		}
		GoodLogger.AutoLevel(constant.ConfigPrefix + ".logger.good")
	})
}
