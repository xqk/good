...
func (y *yaseeDataSource) watch() {
	for {
		...
		// client get err
		if err != nil {
			ilog.Error("yaseeDataSource", ilog.String("listenConfig curl err", err.Error()))
			time.Sleep(time.Second * 1)
			continue
		}
		if resp.StatusCode() != 200 {
			ilog.Error("yaseeDataSource", ilog.String("listenConfig status err", resp.Status()))
			time.Sleep(time.Second * 1)
			continue
		}
		...
		if yaseeRes.Code != 200 {
			ilog.Info("yaseeDataSource", ilog.Int64("code", int64(yaseeRes.Code)))
			time.Sleep(time.Second * 1)
			continue
		}
...
func (y *yaseeDataSource) updateConfig(data string, version int64) {
	...
	case y.changed <- struct{}{}:
		...
		ilog.Info("yaseeDataSource", ilog.String("change", data))
	default:
	}
}
