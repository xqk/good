...
import (
	...
	"github.com/xqk/good/pkg/server/igrpc"
...
func TestApplication_Serve(t *testing.T) {
	Convey("test application serve throw wrong ip", t, func(c C) {
		app := &Application{}
		grpcConfig := igrpc.DefaultConfig()
		grpcConfig.Port = 0
		app.initialize()
...
func TestRegister(t *testing.T) {
	Convey("test application register", t, func(c C) {
		...
		err = app.RegisterHooks(StageBeforeStop, func() error {
			resp, err := etcdctl.Get(context.Background(), "/good/"+pkg.Name()+"/providers/grpc://", clientv3.WithPrefix())
			c.So(err, ShouldBeNil)
			c.So(len(resp.Kvs), ShouldEqual, 1)
			for _, value := range resp.Kvs {
				c.So(string(value.Key), ShouldEqual, "/good/"+pkg.Name()+"/providers/grpc://"+grpcServer.Address())
