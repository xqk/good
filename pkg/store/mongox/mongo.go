...
example: DB := DB("StdConfig", orm.RawConfig("good.mongodb.StdConfig"))
*/
...
func newSession(config Config) *mongo.Client {
	...
	if err != nil {
		_logger.Panic("dial mongo", ilog.FieldAddr(config.DSN), ilog.Any("error", err))
	}
...
func isConfigErr(config Config) {
	if config.SocketTimeout == time.Duration(0) {
		_logger.Panic("invalid config", ilog.FieldExtMessage("socketTimeout"))
	}

	if config.PoolLimit == 0 {
		_logger.Panic("invalid config", ilog.FieldExtMessage("poolLimit"))
	}
}
